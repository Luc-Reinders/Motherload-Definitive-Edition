extends State
class_name PlayerFlashFlightState

@export var player : PlayerFlash

func enter():
	player.animated_sprite.play(AnimatedSpritePlayerFlash.FLIGHT_ANIM)
...
func update(_delta):
	...
	if player.is_on_floor():
		...
		if right and not player.is_facing_right() and player.velocity.x > 0:
			# Turn right
			player.face_right()
			go_to_state(StateMachinePlayerFlash.TURN_FLIGHT_STATE)
